...
function removeModuleAttributes(isStg, env) {
  return {
    ...
    writeBundle(options, bundle) {
      ...
      htmlFiles.forEach(filePath => {
        try {
          ...
          // 本番環境（isStg = false）の場合、絶対パスを相対パスへ変換
          if (!isStg) {
            const assetsPath = (env.ASSETS_PATH ?? '').replace(/^\/+|\/+$/g, '')
            const assetsPrefix = assetsPath ? `${assetsPath}/` : ''

            // scriptタグのsrc属性を調整
            content = content.replace(
              new RegExp(`src="/${assetsPrefix}([^"]*\\.js[^"]*)"`, 'g'),
              'src="./$1?$staticlink$"',
            )
            // linkタグのhref属性を調整（CSSファイル）
            content = content.replace(
              new RegExp(`href="/${assetsPrefix}([^"]*\\.css[^"]*)"`, 'g'),
              'href="./$1?$staticlink$"',
            )
          }
...
export default defineConfig(({ mode, command }) => {
  ...
  const openPath = `/${(env.ASSETS_PATH ?? '').replace(/^\/+|\/+$/g, '')}/${env.DIRECTORY_NAME}.html`
  ...
  return {
    ...
    plugins: [
        ...
        filters: {
          ...
          imgPath: src => {
            if (isStg) {
              ...
              return './' + env.DIRECTORY_NAME + '/img/' + fileName
            }
          },
          ogpPath: src => {
            // Viteの環境変数で判定
      ...
      removeModuleAttributes(isStg, env), // カスタムプラグインに環境変数を渡す
    ],
    css: {
      preprocessorOptions: {
        scss: {
